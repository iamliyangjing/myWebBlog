---
description: 本文系统梳理 MySQL 索引的全貌，从 数据结构、物理存储、字段特性、字段个数 四个维度详细解析索引分类。结合 B+Tree 原理、主键与二级索引的查询机制、覆盖索引与回表、以及联合索引的最左匹配原则与范围查询特殊性，帮助你全面理解索引的设计思想与优化实战。
title: 🚀 MySQL 索引详解与分类 —— B+Tree、聚簇索引与最左匹配原则全解析
tag:
  - MySQL
sidebar: true
comment: true
...
> 索引是为了减少数据库的磁盘 I/O 次数而提出的一种数据结构。

当你想查阅书中某个知识点时，会选择一页页翻找 📖，还是先在目录里查找？
 当然是目录！因为目录能快速定位内容。📑

在数据库中，**索引就好比书的目录**，是一种以空间换时间的设计思想。
 它的核心作用就是：**帮助存储引擎快速获取数据**。

MySQL 常见存储引擎：**InnoDB（默认）、MyISAM、Memory**。其中索引和数据都在存储引擎中实现。
...
常见的索引名词大家肯定都听过：
 聚簇索引、主键索引、二级索引、唯一索引、普通索引、Hash 索引、B+Tree 索引……

但它们如何分类呢？我们可以从 4 个维度来区分：
